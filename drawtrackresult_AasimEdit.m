function drawopt = drawtrackresult_AasimEdit(drawopt, fno, frame, tmpl, param, pts)
...
if (exist('pts') &&(size(pts,2)>1))
  if (size(pts,3) == 1) plot(pts(1,:,1),pts(2,:,1),'yx','MarkerSize',3);end;
  if (size(pts,3) > 1)  plot(pts(1,:,2),pts(2,:,2),'yx','MarkerSize',3);  end;
  if (size(pts,3) > 2)  plot(pts(1,:,3),pts(2,:,3),'rx','MarkerSize',3);  end;
  % 2 dimension(yellow) is truepts and 3rd dimension(red) is tracked pts
end
...
%     imgdisp = cat(3, imgdisp, param.wimg);  
%     str = [str, '   patch'];
%   end
%   if (isfield(param,'err'))
%     imgdisp = cat(3, imgdisp, abs(param.err)*2);  
%     str = [str, '     err'];
% 
%   end
%   if (isfield(param, 'recon'))
%     imgdisp = cat(3, imgdisp, param.recon); 
%     str = [str ,'  recon'];
%   end
...
%   end
%   axis equal tight off;
%      text(5,-4, str, 'Color','y', 'FontWeight','bold', 'FontSize',12);
% end
% 
