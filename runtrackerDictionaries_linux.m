function [dispstr,dispstr1]=runtrackerDictionaries_linux(vidNameW,alphaa,batchsize,patchsize,data,truepts,param0,first,my_mat_x,ErrorFileID,opt,pathtoSaveResults)
...
%%
if (exist('truepts','var') && ~isempty(truepts))
    npts = size(truepts,2);
   opt.npts=npts;
...
drawopt = drawtrackresult_AasimEdit([], 0, frame, tmpl, param, pts);
...
%         Dp is the dictionary and output is the signal representation matrix
...
%         tmpl.basis=Dp;
tmpl.basis=D1;
% if dont update classification Dictionary
%           Dp=D1;
%           Dict.D=[Dp,Dn];
%       %%%%%%    
...
flagtest=0;
if(flagtest)
        if(strcmp(updMethod,'NoUpdate')==0) && (rem(f, opt.batchsize)==0)
            tmplbasis=[];
     [ theta1,thetaVec,totaldiffBasistemp,flag ] = checkUpdateReq(trkptsdiff(f-1),filteredTrkptsDiff(f-1));
       
%            theta(f1)=theta1;
            totaldiffBasis(f)=totaldiffBasistemp;
%             thetaVector(f)=thetaVec;
%             figure(3);  plot(totaldiffBasis,'g.-'),title('TotalDiff');
%             figure(4);  plot(theta,'b.-'),title('Angle between Eigen Spaces');
        end
end
%  flag=1;
           if(flag)
...
%                 trkpts=trackpts(:,:,f-1);
                ...
                [tmpl1,param1,pts0]=reInitializeParams(BB,tmpl,f,frame,trueptsCLM,param,param0,opt);
              param.est=param1.est;
              param.wimg=param1.wimg;
              tmpl.mean=tmpl1.mean;
          end
    ...
    duration = duration + toc;%      if ((exist('truepts','var')) && ~isempty(truepts))
...
%        set(tfig,'rend','painters','pos',[0 0 900 600]) 
    ...
    if (opt.patch)
        ...
        drawopt = drawtrackresult_AasimEdit(drawopt, f, frame, tmplnew, param, pts);
        clear tmplnew;
        clear basi;
        clear sample;
    else
        drawopt = drawtrackresult_AasimEdit(drawopt, f, frame, tmpl, param, pts);
    end
...
plotcorr=0;
if (isfield(opt,'dump') && opt.dump > 0 && plotcorr>0 )
%     cd(vidNameW)
%     pathtoSaveResults='';
vidName=vidNameW;
