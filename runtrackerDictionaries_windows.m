function [dispstr,dispstr1]=runtrackerDictionaries_windows(vidNameW,alphaa,batchsize,patchsize,data,truepts,param0,first,my_mat_x,ErrorFileID,opt)
...
DataFileDir='D:\Aasim\YawDD\YawDD dataset\TestVideosTracking\Results_SibGrapi\ResultswithoutUpdTest';
...
%       %%%%%%    
      currentCoeff=tmpl.basis;
        ...
        flagtest=0;
        flag=0; % If dont want to update, Uncomment this.
        if(flagtest)
...
%             %         trueptsCLMtmp = ReInitializeParamsCLM(trackpts(:,:,f-1),frame,my_mat_x(f,:),options,vidName);
...
%             %     [tmpl,param]=reInitializeParams(my_mat_x,f,tmpl,frame,opt);
%         else
%             opt.numsample=400;
%             opt.affsig = [4,4,.02,.02,.005,.001];
%             opt.batchsize=batchsize;
%         end
...
py1=my_mat_x(f,2)+sy1/2;
    ...
    ErrorCenter(f) = sqrt(sum( (([px1,py1]-param.est(1:2)').^2),2));
