...
%    August 2000.
% Aasim comments Begin
% mu is mean
% Mean=(ff*n0*oldMean+n*newMean)/(n+ff+n0);
% % newMean=mean(AllNewImages)
% % N=Number of images already (A)
% % N0=new number of images (B)
% % ff=Forgetting factor
% % oldMean=mean of old data (mean(A)).}
% Else
% Mean=mean(Allimagesavailable)
% end
% Aasim comments end
...
%   claculate the component of B orthogonal to U
  [q, dummy] = qr(data_res, 0);
%   Old basis and component of B orthogonal to U
  Q = [U0 q];
  R = [ff*diag(D0) data_proj; zeros([size(data,2) length(D0)]) q'*data_res];

  [U,D,V] = svd(R, 0);
%   Rigen vectors of Matrix R is the new eigen vectors
  D = diag(D);
  ...
  D = D(keep);
%   New Basis, which is multiplication  of Q and the old basis
% Look in the paper after equation 1
  U = Q * U(:, keep);
end
